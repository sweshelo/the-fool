...
カード効果を実装するためのヘルパークラス群（効果実行エンジン）：
...
Core (src/package/core)
    ↓ Stack
Card Effects (src/game-data/effects)
...
1. `src/game-data/catalog.ts` にカードデータを追加
2. `src/game-data/effects/cards/[カードID].ts` に効果実装を追加
3. [カード効果実装ガイド](./card-effects/implementation-guide.md) に従って実装

### 新しいイベントの追加

1. `src/game-data/effects/schema/events.ts` にイベント定義を追加
2. `src/game-data/effects/schema/handlers.ts` にハンドラを追加
