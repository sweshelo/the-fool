...
export const effects: CardEffects = {
  ...
  onBootSelf: async (stack: StackWithCard<Unit>): Promise<void> => {
    ...
    if (selfUnits_selectable && opponentUnits_selectable) {
      await System.show(stack, '寒慄の咆哮', '味方と敵それぞれ1体に【沈黙】');

      try {
        // 選択可能なユニットを確認（自分側）
        const filter = (unit: Unit) => (unit.owner.id === owner.id ? true : false);
        // 自分のユニットを選択
        ...
        // 選択可能なユニットを確認（相手側）
        const filter = (unit: Unit) => (unit.owner.id === opponent.id ? true : false);
        // 対戦相手のユニットを選択
