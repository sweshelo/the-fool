...
export const EffectExamples = {
  // 「召喚時に1枚ドローする」効果
  drawOnSummon: async (stack: Stack, card: IAtom, core: Core): Promise<void> => {
    await EffectHelper.showEffect(stack, core, 'ドロー', 'カードを1枚引く')
    await EffectUtils.drawCards(1)(stack, card, core);
  },

  // 「召喚時に相手全体を破壊する」効果
  destroyAllOpponents: async (stack: Stack, card: IAtom, core: Core): Promise<void> => {
    await EffectUtils.breakTargets(EffectUtils.selectAllOpponentUnits)(stack, card, core);
  },

  // 「破壊された時にカードを1枚引く」効果
  drawWhenDestroyed: async (stack: Stack, card: IAtom, core: Core): Promise<void> => {
    await EffectUtils.drawCards(1)(stack, card, core);
  },

  // 「召喚時に対象を選択して破壊する」効果
  destroySelected: async (stack: Stack, card: IAtom, core: Core): Promise<void> => {
    // カードの所有者を特定
    const player = core.players.find(p =>
      p.field.some(c => c.id === card.id) ||
      p.hand.some(c => c.id === card.id)
    ...
    // 対戦相手のフィールド上のカードを取得
    const opponents = core.players.filter(p => p.id !== player.id);
    const opponentUnits: IAtom[] = [];

    for (const opponent of opponents) {
      opponentUnits.push(...opponent.field);
    }

    if (opponentUnits.length === 0) return;

    // プレイヤーに選択を促す
    const selectedTarget = await EffectHelper.promptChoice(
      stack,
      card,
      core,
      player.id,
    ...
    // 選択したカードを破壊
    await EffectUtils.breakTargets(() => Promise.resolve([selectedTarget]))(stack, card, core);
  }
};
